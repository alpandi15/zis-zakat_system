-- Pemilihan periode untuk papan publik /tv dibuat deterministik.
--
-- Sebelumnya periode dipilih dengan `where status = 'active' order by hijri_year desc limit 1`.
-- Bila ada lebih dari satu periode aktif pada tahun hijriah yang sama, urutannya tidak
-- dijamin oleh Postgres sehingga papan /tv bisa menampilkan periode yang berbeda dari
-- yang dipakai aplikasi admin, bahkan berubah antar permintaan.
--
-- Aturan baru: periode aktif didahulukan, lalu tahun hijriah, tahun masehi, tanggal mulai,
-- dan terakhir waktu pembuatan. Bila tidak ada periode aktif sama sekali, papan tetap
-- menampilkan periode terbaru (arsip) alih-alih kosong.
